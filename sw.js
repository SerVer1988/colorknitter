...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    #root { width: 100%; max-width: 440px; min-height: 100vh; display: flex; flex-direction: column; padding: 0 0 110px 0; }

    /* LOGO */
    .logo-row { display: flex; justify-content: center; padding: 10px 0 4px; }
    .logo-row img { width: 52px; height: 52px; border-radius: 12px; }

    /* STATUS */
    .status-box { margin: 6px 12px; border: 1.5px solid #333; border-radius: 999px; padding: 7px 16px; text-align: center; font-size: 13px; color: #ccc; }

    /* SPEED BUTTONS */
    .speed-row { display: flex; justify-content: center; gap: 6px; margin: 6px 12px; }
    .speed-btn { flex: 1; max-width: 60px; padding: 6px 0; border-radius: 999px; border: 1.5px solid #444; background: transparent; color: #aaa; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.15s; }
    .speed-btn.active { background: white; color: black; border-color: white; }

    /* CANVAS */
    .canvas-wrap { display: flex; justify-content: center; padding: 6px 12px; position: relative; }
    canvas { border-radius: 50%; display: block; }
    .sector-labels { position: absolute; top: 6px; left: 12px; right: 12px; bottom: 6px; pointer-events: none; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; }
    .sector-label { font-size: 2rem; font-weight: 900; color: white; text-shadow: 0 0 6px black, 0 0 3px black; display: flex; }
    .sector-label.tl { align-items: flex-start; justify-content: flex-start; padding: 10px 0 0 10px; }
    .sector-label.tr { align-items: flex-start; justify-content: flex-end; padding: 10px 10px 0 0; }
    .sector-label.bl { align-items: flex-end; justify-content: flex-start; padding: 0 0 10px 10px; }
    .sector-label.br { align-items: flex-end; justify-content: flex-end; padding: 0 10px 10px 0; }

    /* STEPS */
    .steps-row { display: flex; justify-content: center; align-items: center; gap: 6px; margin: 8px 12px; }
    .step-item { text-align: center; font-weight: bold; }
    .step-item.cur { border: 2px solid currentColor; border-radius: 12px; padding: 2px 8px; }
    ...
    .bar-btn { background: none; border: none; cursor: pointer; padding: 0; display: flex; align-items: center; justify-content: center; transition: transform 0.15s, opacity 0.15s; }
    .bar-btn:active { transform: scale(0.88); }
    .bar-btn:disabled { opacity: 0.3; }
    .bar-btn img { width: 52px; height: 52px; border-radius: 50%; }
    .bar-btn.play img { width: 68px; height: 68px; }
    .bar-btn.active-radio img { filter: brightness(0) saturate(100%) invert(40%) sepia(80%) saturate(500%) hue-rotate(230deg); }

    /* SETTINGS PANEL */
    .settings-panel { display: none; position: fixed; bottom: 90px; left: 50%; transform: translateX(-50%); width: calc(100% - 24px); max-width: 416px; background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 20px; padding: 16px; z-index: 99; max-height: 70vh; overflow-y: auto; }
    .settings-panel.open { display: flex; flex-direction: column; gap: 14px; }

    .s-label { font-size: 13px; color: #aaa; display: flex; justify-content: space-between; margin-bottom: 4px; }
    ...
    .toggle-slider:before { content:''; position:absolute; width:16px; height:16px; left:4px; top:4px; background:white; border-radius:50%; box-shadow:0 1px 3px rgba(0,0,0,.4); transition:transform 0.2s; }
    ...
    input[type=number] { width: 100%; background: #111; border: 1.5px solid #333; border-radius: 8px; padding: 7px; text-align: center; font-size: 15px; color: white; outline: none; }
    input[type=number]:focus { border-color: #7c3aed; }

    .s-btn { width: 100%; padding: 10px; border-radius: 10px; font-size: 14px; font-weight: 600; cursor: pointer; border: none; }
    .s-btn-outline { background: transparent; border: 1.5px solid #444; color: #ccc; }
    .s-btn-primary { background: #7c3aed; color: white; }
    .s-btn-row { display: flex; gap: 8px; }
    .s-btn-row .s-btn { flex: 1; }
    .s-btn-secondary { background: #222; color: #ccc; border: 1.5px solid #333; }

    .saved-list { display: flex; flex-direction: column; gap: 8px; max-height: 200px; overflow-y: auto; }
    .saved-item { background: #111; border: 1px solid #2a2a2a; border-radius: 10px; padding: 8px 10px; font-size: 12px; }
    .saved-item-top { display: flex; justify-content: space-between; color: #888; margin-bottom: 6px; }
    .saved-item-btns { display: flex; justify-content: flex-end; gap: 8px; }
    .s-btn-sm { padding: 4px 10px; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; border: none; }
    .s-btn-sm-ol { background: transparent; border: 1px solid #444; color: #ccc; }
    .s-btn-sm-del { background: #dc2626; color: white; }

    /* RADIO PANEL */
    .radio-panel { display: none; position: fixed; bottom: 90px; left: 50%; transform: translateX(-50%); width: calc(100% - 24px); max-width: 416px; background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 20px; padding: 16px; z-index: 99; }
    .radio-panel.open { display: block; }
    .radio-title { font-size: 14px; font-weight: 700; color: #e9d5ff; margin-bottom: 10px; }
    .radio-status { font-size: 12px; color: #a78bfa; margin-bottom: 12px; }
    .radio-vol-row { display: flex; align-items: center; gap: 8px; font-size: 14px; }

    .restore-banner { margin: 4px 12px; background: #052e16; border: 1px solid #166534; border-radius: 10px; padding: 7px 12px; font-size: 12px; color: #86efac; text-align: center; }
...
function saveToStorage(data) {
  ...
  catch(e) { try { localStorage.setItem(STORAGE_KEY, JSON.stringify({...data, circleImage: null})); } catch(_) {} }
}
function loadFromStorage() {
  try { const s = localStorage.getItem(STORAGE_KEY); return s ? JSON.parse(s) : null; } catch(_) { return null; }
}
...
function ColorKnitter() {
  ...
  const [wasRestored]                   = useState(!!restored?.sequence?.length);
  ...
  useEffect(() => { if (audioRef.current) audioRef.current.volume = radioVolume; }, [radioVolume]);

  useEffect(() => {
    if (restored?.circleImage) {
      const img = new Image();
      img.onload = () => { loadedImgRef.current = img; };
      img.src = restored.circleImage;
    }
  }, []);

  useEffect(() => {
    saveToStorage({ sequence, currentIndex, speedMultiplier, targetSteps, findStep,
      savedStates, remainingSteps, showNextLine, circleImage, imageOpacity });
  ...
  const calcElapsed = () => {
    const secPerStep = (6 - speedMultiplier) / 1.1;
    return Math.round(currentIndex * secPerStep);
  };
  ...
  const checkGoal = () => {
    if(targetSteps>0){
      stepCountRef.current+=1; setRemainingSteps(targetSteps-stepCountRef.current);
  ...
  useEffect(()=>{
    ...
    ctx.beginPath(); ctx.arc(cx,cy,radius,0,2*Math.PI); ctx.fillStyle='white'; ctx.fill();
    ...
    ctx.beginPath(); ctx.arc(cx,cy,radius,0,2*Math.PI); ctx.strokeStyle='#333'; ctx.lineWidth=1; ctx.stroke();
  ...
  const statusText = () => {
    if(!sequence.length)return'Загрузите файл';
  ...
  const togglePanel = (panel) => {
    if(panel==='settings'){setShowSettings(p=>!p);setShowRadio(false);}
    if(panel==='radio'){setShowRadio(p=>!p);setShowSettings(false);}
  };
  ...
  return(
    <div id="app">
      ...
      <div className="logo-row">
        <img src="icon.png" alt="CK" onError={e=>e.target.style.display='none'} />
      </div>
      ...
      <div className="speed-row">
        {SPEEDS.map(s=>(
          <button key={s} className={`speed-btn${speedMultiplier===s?' active':''}`}
      ...
      <div className={`settings-panel${showSettings?' open':''}`}>
        ...
        <div className="grid2">
          ...
          <div>
            ...
            <input type="number" value={findStep} min={1} max={sequence.length||1}
              onChange={e=>{const v=Number(e.target.value);if(v>=1&&v<=sequence.length){setFindStep(v);setCurrentIndex(v-1);speak(sequence[v-1]);}}} />
          </div>
        </div>
      ...
      <div className="bottom-bar">
        <button className={`bar-btn${radioPlaying?' active-radio':''}`} onClick={()=>setRadioPlaying(p=>!p)}>
          <img src="ic_music.png" alt="radio" />
        </button>
        ...
        <button className="bar-btn play" onClick={()=>setIsPlaying(p=>!p)} disabled={!sequence.length}>
          <img src={isPlaying?'ic_pause.png':'ic_play.png'} alt="play" />
        </button>
        <button className="bar-btn" onClick={()=>moveSteps(1)} disabled={currentIndex>=sequence.length-1}>
          <img src="ic_next.png" alt="next" />
        </button>
        <button className={`bar-btn${showSettings?' active-radio':''}`} onClick={()=>togglePanel('settings')}>
          <img src="ic_settings.png" alt="settings" />
        </button>
      </div>
    </div>
  );
